/**
 * Gemini API service for generating training plans and insights
...
/**
 * Create a prompt for Gemini based on user inputs and training data
 ...
 * @returns {Object} - Prompt object for Gemini API
 */
export const createGeminiPrompt = (formData, trainingDataSummary) => {
...
/**
 * Parse Gemini API response into structured insights and training plan
...
export const parseGeminiResponse = (response) => {
  try {
    ...
    return {
      ...
      rawResponse: response?.candidates?.[0]?.content?.parts?.[0]?.text || "No response text"
    };
  }
};

/**
 * Call Gemini API to generate a training plan
...
export const generateTrainingPlan = async (formData, activitiesData) => {
  try {
    ...
    const promptData = createGeminiPrompt(formData, trainingDataSummary);
